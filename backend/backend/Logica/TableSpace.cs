        ...
        public ResDeleteTableSpace DeleteTableSpace(ReqDeleteTableSpace request)
        {
            ...
            try
            {
                using (OracleConnection connection = new OracleConnection(_connectionString))
                {
                    ...
                    string dropSql = $"DROP TABLESPACE \"{request.TableSpaceName}\" INCLUDING CONTENTS AND DATAFILES";
                    using (OracleCommand dropCmd = new OracleCommand(dropSql, connection))
                    {
